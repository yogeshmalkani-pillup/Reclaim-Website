...
export default function Feature({ hidden, title, description, image }) {
    return (
        <div className={`flex flex-row max-md:flex-col gap-20 max-sm:gap-10 items-center justify-center w-full h-full relative ${hidden ? 'hidden' : ''} `}>
            ...
            <div key={title} className={`flex flex-col max-md:gap-5 gap-10 w-[40%] max-md:w-full items-x  ${styles.feature} ${hidden ? 'hidden' : ''}`}>
                <h2 className='text-[3vw] max-sm:text-[30px] max-md:text-center  font-satoshi_bold text-[#3CE8B5]'>{title}</h2>
                <p className='text-[2vw] max-sm:text-[20px] max-md:text-center  font-satoshi_medium'>
                    {description}
                </p>
            </div>
        </div>
