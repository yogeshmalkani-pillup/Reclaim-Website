...
export default function Navbar({ activeSection, scrollToView }) {
    ...
    return (
        <div
            className={`w-full flex flex-row fixed z-50  justify-around px-20 mt-5  max-sm:px-5   ${mulish.className} `}
        >
            <div
                className={`w-[70%] rounded-[120px] py-3 px-5  flex-row justify-between h-[80px] ${styles.glassEffect} hidden md:flex`}
            >
                            ...
                            {Sections.map((sec) => {
                                return (
                                    <p
                                        ...
                                        key={sec}
                                        className={`${activeSection == sec ? " font-bold" : "font-thin"
                ...
                <ShimmerBorder
                    containerClass="w-fit p-[2px]"
                    textClass="px-4 py-2 max-sm:py-2 bg-black text-[16px] max-sm:text-[20px] font-satoshi_medium  text-white"
                >
                    <p>Try for free</p>
                </ShimmerBorder>
            </div>
            <div
                className={`w-[90%] ${navExpand ? "rounded-2xl" : "rounded-[100px]"} ${styles.glassEffect
                    } px-3 py-3  flex-col justify-between items-center min-h-[8vh] flex  md:hidden`}
            >
                ...
                {navExpand && (
                    <div className="flex flex-col justify-center items-center gap-5 w-full mt-10 p-5">
                        {Sections.map((sec) => {
                            return (
                                <button
                                    onClick={() => {
